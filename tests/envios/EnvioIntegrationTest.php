...
class EnvioIntegrationTest extends TestCase
{
    ...
    {
        $orderId = $this->crearOrdenConEnvio('Flat Shipping Rate', 'flat.flat', 15.00);
        $orden = $this->obtenerOrdenEnvio($orderId);

        $this->assertSame('Flat Shipping Rate', $orden['shipping_method']);
        $this->assertSame('flat.flat', $orden['shipping_code']);
    ...
    /** @test */
    public function testCambioDeMetodoDeEnvioActualizaCodigoYMonto(): void
    {
        $orderId = $this->crearOrdenConEnvio('Flat Shipping Rate', 'flat.flat', 15.00);

        $this->actualizarEnvio($orderId, 'Pickup', 'pickup.pickup', 0.00);
        $orden = $this->obtenerOrdenEnvio($orderId);

        $this->assertSame('Pickup', $orden['shipping_method']);
        $this->assertSame('pickup.pickup', $orden['shipping_code']);
    ...
    {
        $orderId = $this->crearOrdenConEnvio('Flat Shipping Rate', 'flat.flat', 15.00);
        $this->actualizarEnvio($orderId, 'Express', 'express.express', 25.00);
    ...
    private function crearOrdenConEnvio(string $shippingMethod, string $shippingCode, float $shippingValue): int
    {
        $subtotal = 100.00;
        ...
        $sql = "INSERT INTO {$this->prefix}order
            ...
            (0, '', 0, 'OpenCart', 'http://127.0.0.1:8000/', 0, 1,
             ...
             'Cash On Delivery', 'cod', 'Ship', 'Tester', '', 'Street 1', '', 'Lima', '15001', 'Lima', 0, 'Peru',
             0, '', '', :shipping_method, :shipping_code, '', :total, 1, 0, 0.0000, 0, '', 1, 1,
        ...
        $insertTotal = $this->db->prepare("INSERT INTO {$this->prefix}order_total (order_id, extension, code, title, value, sort_order) VALUES (:order_id, 'sub_total', 'sub_total', 'Sub-Total', 100.00, 1), (:order_id, 'shipping', 'shipping', 'Shipping', :shipping, 3), (:order_id, 'total', 'total', 'Total', :total, 9)");
        $insertTotal->execute([':order_id' => $orderId, ':shipping' => $shippingValue, ':total' => $total]);

        return $orderId;
    }

    private function actualizarEnvio(int $orderId, string $shippingMethod, string $shippingCode, float $shippingValue): void
    {
        $updateOrder = $this->db->prepare("UPDATE {$this->prefix}order SET shipping_method = :method, shipping_code = :code, total = 100.00 + :shipping WHERE order_id = :order_id");
        $updateOrder->execute([':method' => $shippingMethod, ':code' => $shippingCode, ':shipping' => $shippingValue, ':order_id' => $orderId]);

        $updateShipping = $this->db->prepare("UPDATE {$this->prefix}order_total SET value = :shipping WHERE order_id = :order_id AND code = 'shipping'");
        $updateShipping->execute([':shipping' => $shippingValue, ':order_id' => $orderId]);

        $updateTotal = $this->db->prepare("UPDATE {$this->prefix}order_total SET value = 100.00 + :shipping WHERE order_id = :order_id AND code = 'total'");
        $updateTotal->execute([':shipping' => $shippingValue, ':order_id' => $orderId]);
    }

    private function obtenerOrdenEnvio(int $orderId): array
    {
        $stmt = $this->db->prepare("SELECT shipping_method, shipping_code, shipping_address_1 FROM {$this->prefix}order WHERE order_id = :order_id");
